...
const sound = new Audio("sonidos/hithato1.wav");
const soundplay = () => {
  sound.currentTime = 0;
  sound.play();
};

class Hithatopen extends HTMLElement {
  constructor() {
    super();
  ...
  static get styles() {
    ...
    .cymbals.animated .cymbal.top {
      animation: movecymbal 0.40s ease;
    }
    @keyframes movecymbal {
        0%, 50%{transform: translateY(-3px);}
  ...
  render() {
    this.shadowRoot.innerHTML = /* html */ `
      <style>${Hithatopen.styles}</style>
      <div class="cymbals">
       <div class="cymbal top"></div>
       <div class="cymbal bottom"></div>
      </div>
      <base-tripode class="medio"></base-tripode>
   
     `;
  }
}

customElements.define("hithat-open", Hithatopen);
